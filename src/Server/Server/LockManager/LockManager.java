...
public class LockManager
{
	...
	private static int DEADLOCK_TIMEOUT = 1000000;
				...
				synchronized (this.lockTable) {
					...
					if (!bConflict) {
						...
						if (bConvert.get(0) == true) {
							// Step 1 remove the old read lock
							TransactionLockObject xLockObjectRead = new TransactionLockObject(xid, data,  TransactionLockObject.LockType.LOCK_READ);
							DataLockObject dataLockObjectRead = new DataLockObject(xid, data,  TransactionLockObject.LockType.LOCK_READ);
							this.lockTable.remove(xLockObjectRead);
							this.lockTable.remove(dataLockObjectRead);

							// Step 2 add a write lock
