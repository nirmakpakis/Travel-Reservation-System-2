	...
	public void execute(Command cmd, Vector<String> arguments) throws RemoteException, NumberFormatException {
		switch (cmd) {
			case Shutdown: {
				if (arguments.size() == 1) {
					...
					System.err.println((char) 27 + "[31;1mCommand exception: " + (char) 27
							+ "[0mImproper use of Shutdown command.");
				}
				break;
			}
			case Help: {
			...
			case Commit: {
				if (arguments.size() != 2) {
					System.err.println(
							(char) 27 + "[31;1mCommand exception: " + (char) 27 + "[0mImproper use of Commit command.");
				} else {
					String result = m_resourceManager.commit(Integer.parseInt(arguments.elementAt(1)));
			...
			case Abort: {
				if (arguments.size() != 2) {
					System.err.println(
							(char) 27 + "[31;1mCommand exception: " + (char) 27 + "[0mImproper use of Abort command.");
				} else {
